    ...
    public class Bat : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (_onAir)
            {
                ...
                _width = Mathf.Cos(Time.time * _maxMinHeight);
                if (_width >= 0.95f)
                {
                    _width *= -1;
                }
            ...
            if (_timeFlying > 0f)
            {
                ...
                //Debug.Log(Vector3.Distance(_targetPosition, transform.position));
                //
                
                if (Vector3.Distance(_targetPosition, transform.position) <= Mathf.Abs(_distance) + 1)
                {
                    Debug.Log("sddddddddddddddddddd");
                    _targetPosition = new Vector3(transform.position.x + _distance * _speed, transform.position.y + _height, 0);
                }
                else if (_onAir)
                {
        ...
        private IEnumerator StartToMove()
        {
            ...
            _onAir = true;
            _stand = false;
            _targetPosition = new Vector3(transform.position.x, transform.position.y - 4.2f, 0);
            Fly();
        }
